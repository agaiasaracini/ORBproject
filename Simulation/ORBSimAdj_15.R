...
for (i in 1:nrow(parDATA)) {
  ...
  cat("The", i, "th parameter combination out of", nrow(parDATA), "initiated.\n")
  
  tryCatch({
    
    # Extract parameter values
    mu <- parDATA$mu_values[i]
    k <- parDATA$k_values[i]
    tau_squared <- parDATA$tau_squared_values[i]
    gamma <- parDATA$gamma[i]
    ...
    for (j in 1:n_sim) {
      tryCatch({
        result <- simulate_ORB_data(n_studies = k,
                                    mu = mu,
    ...
    res <- foreach(meta_data_miss = datasets, .options.RNG = seed, .combine = 'c') %dorng% {
      ...
      resultDGM <- reORBadj(y = meta_data_miss$y_ORB,
                            s = meta_data_miss$s_ORB,
                            n1 = meta_data_miss$n_treatment,
                            n2 = meta_data_miss$n_control,
                            outcome = "benefit",
    ...
    resultDGM_list <- formatting(res, "DGM")
    ...
    theta_hat_DGM <- extract(resultDGM_list, "mu_adjusted_benefit")
    theta_hat_low_DGM <- extract(resultDGM_list, "LR_mu_adjusted_low")
    theta_hat_up_DGM <- extract(resultDGM_list, "LR_mu_adjusted_up")
    tau2_hat_DGM <- extract(resultDGM_list, "tau_squared_adjusted")
    tau2_hat_low_DGM <-extract(resultDGM_list,  "LR_tau_squared_adjusted_low")
    tau2_hat_up_DGM <- extract(resultDGM_list, "LR_tau_squared_adjusted_up")
    ...
    summaryDGM <- calculate_summary(theta_hat_DGM, theta_hat_low_DGM, theta_hat_up_DGM, mu)
    summaryUnadj <- calculate_summary(theta_hat_Unadj, theta_hat_low_Unadj, theta_hat_up_Unadj, mu)
    
    # Store average results in parDATA dataframe
    parDATA[i, paste0("LL_", names(summaryLL))] <- unlist(summaryLL)
    parDATA[i, paste0("LC_", names(summaryLC))] <- unlist(summaryLC)
    parDATA[i, paste0("CL_", names(summaryCL))] <- unlist(summaryCL)
    parDATA[i, paste0("CC1_", names(summaryCC1))] <- unlist(summaryCC1)
    parDATA[i, paste0("CC2_", names(summaryCC2))] <- unlist(summaryCC2)
    parDATA[i, paste0("DGM_", names(summaryDGM))] <- unlist(summaryDGM)
    parDATA[i, paste0("Unadj_", names(summaryUnadj))] <- unlist(summaryUnadj)
    ...
    summaryDGM_tau2 <- calculate_summary(tau2_hat_DGM, tau2_hat_low_DGM, tau2_hat_up_DGM, tau_squared)
    summaryUnadj_tau2 <- calculate_summary(tau2_hat_Unadj, tau2_hat_low_Unadj, tau2_hat_up_Unadj, tau_squared)
    
    # Store average results in parDATA dataframe
    parDATA[i, paste0("tau2_LL_", names(summaryLL_tau2))] <- unlist(summaryLL_tau2)
    parDATA[i, paste0("tau2_LC_", names(summaryLC_tau2))] <- unlist(summaryLC_tau2)
    parDATA[i, paste0("tau2_CL_", names(summaryCL_tau2))] <- unlist(summaryCL_tau2)
    parDATA[i, paste0("tau2_CC1_", names(summaryCC1_tau2))] <- unlist(summaryCC1_tau2)
    parDATA[i, paste0("tau2_CC2_", names(summaryCC2_tau2))] <- unlist(summaryCC2_tau2)
    parDATA[i, paste0("tau2_DGM_", names(summaryDGM_tau2))] <- unlist(summaryDGM_tau2)
    parDATA[i, paste0("tau2_Unadj_", names(summaryUnadj_tau2))] <- unlist(summaryUnadj_tau2)
...
# If any error occurred, return parDATA until that point
if (exists("i") && i < nrow(parDATA)) {
  parDATA <- parDATA[1:i, ]
}

# Save results
saveRDS(parDATA, file = "AdjustResults3200k_15.rds")
