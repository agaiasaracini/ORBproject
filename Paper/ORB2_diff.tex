...
%DIF ADD ORB2_Revised.tex   Thu Jul 24 22:05:53 2025
...
. The most established }\DIFdelend \DIFaddbegin \DIFadd{of \mbox{%DIFAUXCMD
...
\DIFadd{In contrast, the most established MNAR-based }\DIFaddend ORB-adjustment \DIFdelbegin \DIFdel{method, i.e., that of \mbox{%DIFAUXCMD
\citet{Copas2019}}\hskip0pt%DIFAUXCMD
, relies on }\DIFdelend \DIFaddbegin \DIFadd{is the method proposed by \mbox{%DIFAUXCMD
\citet{Copas2019, Copas2014}}\hskip0pt%DIFAUXCMD
, which adjusts each outcome separately and explicitly models the missing data mechanism. The method relies on first }\DIFaddend categorizing unreported outcomes into risk \DIFdelbegin \DIFdel{of bias }\DIFdelend categories - no risk (NR), low risk (LR), and high risk (HR) - based on the Outcome Reporting Bias in Trials (ORBIT) \DIFdelbegin \DIFdel{methodology. Given the classification , assumed to be correct, \mbox{%DIFAUXCMD
\citet{Copas2019} }\hskip0pt%DIFAUXCMD
...
This is thus the generic form for \DIFdelbegin \DIFdel{ORB adjustment}\DIFdelend \DIFaddbegin \DIFadd{ORB-adjustment}\DIFaddend , which has different shapes depending on the selection function $w(y_i)$ used, representative of the missing \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{not at random (MNAR) }\DIFaddend mechanism assumed for unreported study outcomes. Given the alignment of our \DIFdelbegin \DIFdel{ORB adjustment }\DIFdelend \DIFaddbegin \DIFadd{ORB-adjustment }\DIFaddend with the PB framework of selection models, one can use similar selection functions which are typically found in the PB literature. 
...
While this selection function can be found in the PB literature \citep{selection0, selection1, reviewselection}, we note that it is also the one implicitly used in the \citet{Copas2019} adjustment, although the authors do not explicitly frame the \DIFdelbegin \DIFdel{ORB adjustment }\DIFdelend \DIFaddbegin \DIFadd{ORB-adjustment }\DIFaddend via a selection model framework. Of note, in \citet{Copas2019}, \DIFdelbegin \DIFdel{ORB adjustment }\DIFdelend \DIFaddbegin \DIFadd{ORB-adjustment }\DIFaddend is applied by including only the unreported study outcomes classified at HR of bias by the ORBIT classification system. They thus omit the unreported study outcomes classified at LR of bias and regard them as missing at random. Furthermore, the authors use the two-sided $p$-value $p = 2 \cdot (1 - \Phi(\abs{\frac{y}{\sigma}}))$ instead of the one-sided one proposed in this work. We deem a one-sided $p$-value to be more appropriate to model the underlying missing data mechanism for a beneficial effect of treatment, as it would be unlikely for significant outcomes, but in the wrong direction, to be reported \citep{HedgesVev, reviewselection, selectionCont}. 
