...
%DIF ADD ORB2_Revised.tex   Tue Jun 24 03:53:23 2025
...
. Although these strategies offer partial improvements under MNAR, they generally do not model the MNAR mechanism directly and may inadvertently introduce bias into outcomes that are fully observed due to the joint modeling assumptions.
}
...
\citet{Copas2019} }\hskip0pt%DIFAUXCMD
method, it is assumed that treatment effects, and possibly standard errors, are unreported, while sample sizes of the studies are known, and the adjustment is done separately for each outcome in the meta-analysis}\DIFdelend \DIFaddbegin \DIFadd{Of note, this a classification specific to ORB in that it assigns a risk of bias evaluation to each unreported outcome within a study, rather than to the whole study itself. The latter is a risk of bias approach typical of PB \mbox{%DIFAUXCMD
\citep{Lunny2024,RiskBias, Begg, Salanti} }\hskip0pt%DIFAUXCMD
but does not directly apply to ORB, in that studies are published but different unreported outcomes may be associated with different risks of ORB within the same study \mbox{%DIFAUXCMD
...
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Selection Functions Implemented}} \label{ourSELs}
\DIFaddend 
...
\DIFadd{We apply the ORB-adjustment framework to the epilepsy meta-analysis data affected by ORB from \mbox{%DIFAUXCMD
\citet{topiramate, Copas2019} }\hskip0pt%DIFAUXCMD
...
\begin{figure*}[!hbt]
\centering
\caption{\DIFaddFL{Application of ORB-adjustment sensitivity analysis to example epilepsy data \mbox{%DIFAUXCMD
\citep{Copas2019, topiramate}}\hskip0pt%DIFAUXCMD
...
noted some }\DIFdelend \DIFaddbegin \DIFadd{the explicit modelling of the missing data mechanism, characteristic of the selection model framework, into the multivariate meta-analysis frameworks \mbox{%DIFAUXCMD
