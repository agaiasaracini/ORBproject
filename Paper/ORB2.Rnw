...
\label{ith.high}
\begin{aligned}
f\left(y_i \mid \, i \in \{\text{Unrep} \} \right) = \frac{f \left(y_i \text{;} \theta \right) \cdot \left(1-w(y_i) \right)}{\int_{-\infty}^{\infty} f \left(y\right \text{;} \theta) \cdot \left(1-w(y) \right) d y} \text{.}
\end{aligned}
\end{equation}
