...
p1 <- ggplot(data, aes(x = Method, y = Estimate, ymin = CI_Low, ymax = CI_High)) +
  ...
  labs(title = " ",
       x = " ",
       y = "\nlog RR 50% Seizure Frequency Reduction\n") +
  scale_color_manual(
  ...
  scale_y_continuous(limits=c(0.1,2), breaks= seq(0.6, 1.4,  by=0.2))+
...
geom_text(aes(x = "Naive", y = 1.7, label = "Naive"), parse = TRUE, 
           hjust = +0.05, 
           vjust = -0.5,
            size = 2.2,
             position = position_nudge(x = -0.2))+
  geom_text(aes(x = "LL", y = 1.7, label = paste0(expression(w[A]))), parse = TRUE, 
            hjust = +0.05, 
                     vjust = -0.5,
            size = 2.2,
             position = position_nudge(x = -0.2))+
    geom_text(aes(x = "LC", y = 1.7, label = paste0(expression(w[B](gamma == 3)))), parse = TRUE, 
            hjust = +0.05, 
                     vjust = -0.5,
            size = 2.2,
             position = position_nudge(x = -0.2))+
  geom_text(aes(x = "CL", y = 1.7, label = paste0(expression(w[C](gamma == 3)))), parse = TRUE, 
           hjust = +0.05, 
                     vjust = -0.5,
            size = 2.2,
             position = position_nudge(x = -0.2))+
  geom_text(aes(x = "CC2", y = 1.7, label = paste0(expression(w[D](gamma == 7 ~ "," ~ beta == 1.5)))), parse = TRUE, 
           hjust = +0.05, 
                     vjust = -0.5,
            size = 2.2,
             position = position_nudge(x = -0.2))+
  geom_text(aes(x = "CC1", y = 1.7, label = paste0(expression(w[D](gamma == 1.5 ~ "," ~ beta == 7.5)))), parse = TRUE, 
           hjust = +0.05, 
                     vjust = -0.5,
            size = 2.2,
             position = position_nudge(x = -0.2))
...
final_plot <- grid.arrange(
  ...
  nrow = 1,
  widths = c(4.8, 0.7),
  left = grid::textGrob(expression(E(hat(mu)) - mu), rot=90, gp = grid::gpar(fontsize = 10)), 
