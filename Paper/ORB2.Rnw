...
\label{ith.rep}
\begin{aligned}
f\left(y_i \mid  \, i \in \{\text{Rep}\} \right) = \frac{f\left(y_i \text{;} \theta \right) \cdot w(y_i)}{\int_{-\infty}^{\infty} f\left(y \text{;} \theta \right) \cdot w(y) d y} \text{.}
\end{aligned}
\end{equation}
...
\label{ith.high}
\begin{aligned}
f\left(y_i \mid \, i \in \{\text{Unrep} \} \right) = \frac{f \left(y_i \text{;} \theta \right) \cdot \left(1-w(y_i) \right)}{\int_{-\infty}^{\infty} f \left(y \text{;} \theta \right) \cdot \left(1-w(y) \right) d y} \text{.}
\end{aligned}
\end{equation}
...
0 & \text{if } p_i > \alpha \text{,} \end{cases}
\end{aligned}
\end{equation}
...
\begin{figure*}[!hbt]
\centering
\caption{Application of ORB-adjustment to example data \citep{Copas2019, topiramate}, using the different selection functions showcased in Figure \ref{new.weight.fig}. In addition, the naive estimate, without ORB-adjustment, is shown for comparison. The plot shows the risk ratio (RR) estimates and $95\%$ profile likelihood (PL) confidence intervals (CI).}
%\hline
...
The $\texttt{R}$ code for the simulation study presented in this research can be accessed in the \href{https://github.com/agaiasaracini/ORBproject}{agaiasaracini/ORBproject GitHub repository}. This repository includes all the scripts used in the simulation, featuring the key function $\texttt{reORBadj}$, which implements the ORB-adjustment following the selection model framework described in this study. This function is thus applicable to any meta-analysis dataset with unreported study outcomes that might indicate ORB. Additionally, the repository contains the final output data from the simulation study used to showcase our results, along with the i) main manuscript, ii) supplementary material, and iii) simulation study protocol, all of which are dynamically executable. This research emphasizes ensuring reproducibility and transparency in scientific investigations.
