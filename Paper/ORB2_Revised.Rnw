...
From a missing data perspective, ORB reflects a missing not at random (MNAR) scenario—meaning that the probability an outcome is unreported depends on its value, typically its statistical significance. This contrasts with the assumption of missing completely at random (MCAR), where missingness is unrelated to the data. Standard meta-analytic methods often implicitly assume MCAR by including only reported outcomes, leading to biased estimates when the true mechanism is MNAR.

A related but distinct setting is missing at random (MAR), where the reporting of one outcome depends on other observed outcomes within the same study. This is particularly relevant in multivariate meta-analysis, where methods have been developed to leverage correlations between outcomes to borrow strength across them \citep{Kirkham2012, Bay, dutch, Hwang2018, Liu2017}. Notable examples include the bivariate meta-analysis of \citet{Kirkham2012} and its Bayesian extension by \citet{Bay}, as well as expansions to indirect treatment comparisons and network meta-analysis by \citet{Hwang2018} and \citet{Liu2017}. These approaches can reduce bias under MAR, especially when outcomes are strongly correlated. However, estimating the between- and within-study correlation structures is often difficult. Some methods address this by assuming a global correlation coefficient \citep{Kirkham2012, Bay, dutch, Hwang2018}, estimated either via Pearson’s correlation \citep{Kirkham2012, dutch}, or, in the case of \citet{Liu2017} using copula models like the Clayton copula or via non-informative priors in a Bayesian setting \citep{Bay, Liu2017, Hwang2018}. Although these strategies offer partial improvements under MNAR, they generally do not model the MNAR mechanism directly and may inadvertently introduce bias into outcomes that are fully observed due to the joint modeling assumptions.

In contrast, the most established MNAR-based ORB adjustment is the method proposed by \citet{Copas2019, Cpas2014}, which adjusts each outcome separately and explicitly models the missing data mechanism. Their method relies on first categorizing unreported outcomes into risk categories—no risk (NR), low risk (LR), and high risk (HR)—based on the Outcome Reporting Bias in Trials (ORBIT) classification system. Assuming these categories are correctly assigned, their likelihood-based model includes contributions from unreported HR outcomes, under the assumption that these would have been non-significant. Of note, this a classification specific to ORB in that it assigns a risk of bias evaluation to each unreported outcome within a study, rather than to the whole study itself. The latter is a risk of bias approach typical of PB \citep{Lunny2024,RiskBias, Begg, Salanti} but does not directly apply to ORB, in that studies are published but different unreported outcomes may be associated with different risk of ORB \citep{Copas2019, Copas2014, Kirkham2012}.

Our work builds on this foundation by reinterpreting ORB adjustment through the lens of selection models, a framework commonly used for PB adjustment \citep{selection2, selection1, reviewselection}. Our proposed method removes the need for ORBIT classification, allows flexible modeling of the missing data mechanism, and incorporates all unreported outcomes in the analysis, not just the ones classified as HR of bias by the ORBI methodology. We also investigate the influence of between-study heterogeneity - a novel aspect in the ORB literature - and assess the effectiveness of our method through simulations. The focus is on treatment effect estimation, with a secondary emphasis on how heterogeneity interacts with ORB and its correction. 
...
\subsection{Selection Functions Implemented}
...
\subsubsection{Constant-variable}
...
\subsubsection{Variable-constant}
...
\subsubsection{Variabl-variable}
...
When utilizing any of the selection functions presented in the ORB-adjusted log-likelihood \eqref{lik.rewritten.special.copas.case}, we require knowledge of the standard error of the unreported study outcome, which is generally missing. This value hence needs to be imputed; we follow the methodology of \citet{Copas2019} and \citet{Bay} and impute the missing standard error of an unreported study $i$ as
...
final_final_plot <- gridExtra::grid.arrange(arrangeGrob(main_plot0, 
  ...
  nrow = 1,
  widths = c(5, 1),
  left = grid::textGrob("Probability of reporting", rot = 90, gp = grid::gpar(fontsize = 10)),
  bottom = grid::textGrob("One-sided p-value", gp = grid::gpar(fontsize = 10)))
          ...
          if (length(sigma_squared_imputed) > 0) {
            ...
            if (sel.ben == "Constant.Constant"){
              ...
              TT <- function(y, sigma_squared_imputed, alpha_ben) {
                ...
                ifelse(p <= alpha_ben, 
                       1 - (1-wprob)*(p^rho1)/(alpha_ben^rho1),
                       wprob*(p^(-rho2))/(alpha_ben^(-rho2))
                )
              }
...
# Grid of rho1 and rho2 values beta gamma
rho1_vals <- c(0.001, 0.5, 1, 1.5, 2, 2.5, 3)
rho2_vals <- c(0.001, 0.5, 1, 1.5, 2, 2.5, 3)

gr <- expand.grid(rho1 = rho1_vals, rho2 = rho2_vals)
...
# Function to safely run reORBadj
safe_reORBadj1 <- function(r1, r2) {
  res <- tryCatch({
    out <- reORBadj(
      a = meta_data_miss$a_SeizureRed,
      c = meta_data_miss$c_SeizureRed,
      n1 = meta_data_miss$Nt,
      n2 = meta_data_miss$Nc,
      outcome = "benefit",
    ...
    data.frame(rho1 = r1, rho2 = r2,
               mu = out$mu_adjusted,
               ci_low = out$LR_mu_adjusted_low,
               ci_up = out$LR_mu_adjusted_up)
  }, error = function(e) {
    data.frame(rho1 = r1, rho2 = r2, mu = NA, ci_low = NA, ci_up = NA)
  })
  return(res)
}

# Apply function over grid
resultsSENS1 <- purrr::map2_df(gr$rho1, gr$rho2, safe_reORBadj1)

# Function to safely run reORBadj
safe_reORBadj21 <- function(r1, r2) {
  res <- tryCatch({
    out <- reORBadj(
      a = meta_data_miss$a_SeizureFree,
      c = meta_data_miss$c_SeizureFree,
      n1 = meta_data_miss$Nt,
      n2 = meta_data_miss$Nc,
      outcome = "benefit",
...
# Apply function over grid
resultsSENS21 <- purrr::map2_df(gr$rho1, gr$rho2, safe_reORBadj21)


############# w=0.5 ######################

# Function to safely run reORBadj
safe_reORBadj <- function(r1, r2) {
  res <- tryCatch({
    out <- reORBadj(
      a = meta_data_miss$a_SeizureRed,
      c = meta_data_miss$c_SeizureRed,
...
# Apply function over grid
resultsSENS <- purrr::map2_df(gr$rho1, gr$rho2, safe_reORBadj)

# Function to safely run reORBadj
safe_reORBadj2 <- function(r1, r2) {
  res <- tryCatch({
    out <- reORBadj(
      a = meta_data_miss$a_SeizureFree,
      c = meta_data_miss$c_SeizureFree,
...
# Apply function over grid
resultsSENS2 <- purrr::map2_df(gr$rho1, gr$rho2, safe_reORBadj2)


########## w=0 #########################

# Function to safely run reORBadj
safe_reORBadj0 <- function(r1, r2) {
  res <- tryCatch({
    out <- reORBadj(
      a = meta_data_miss$a_SeizureRed,
      c = meta_data_miss$c_SeizureRed,
...
# Apply function over grid
resultsSENS0 <- purrr::map2_df(gr$rho1, gr$rho2, safe_reORBadj0)

# Function to safely run reORBadj
safe_reORBadj20 <- function(r1, r2) {
  res <- tryCatch({
    out <- reORBadj(
      a = meta_data_miss$a_SeizureFree,
      c = meta_data_miss$c_SeizureFree,
...
# Apply function over grid
resultsSENS20 <- purrr::map2_df(gr$rho1, gr$rho2, safe_reORBadj20)
...
It is important to note that while our simulation assumes a known missingness mechanism—selective reporting based on a continuous function of the one-sided p-value—no information about the cause of missingness is assumed to be available once the data are generated. This reflects the practical setting in which some outcomes are unreported, but their classification into high or low risk of bias is unknown. This differs from our previous simulation approach \citep{mythesis}, where unreported outcomes were classified using ORBIT into high or low risk of bias based on significance. That design allowed a direct comparison with the method of \citet{Copas2019}, which requires such classification to adjust only for outcomes deemed at high risk. In contrast, our current simulation avoids this additional layer of information and instead models ORB as a continuous function of statistical significance, consistent with the selection model literature. As a result, the \citet{Copas2019} method is not applicable in this setting, but in previous work we found similar performance when comparing both approaches under aligned assumptions \citep{mythesis}.
...
\subsection{Application to Example Data}
...
Since the true underlying data mechanism is unknown in this example, we recommend to adopt a sensitivity analysis approach to the ORB-adjustment. Figure \ref{exampleSENS} showcases the ORB-adjustment using three different selection functions \eqref{special1}, \eqref{special2} and \eqref{special3}, for a range of values of $\gamma$ and $\beta$ parameters.
...
geom_text(aes(x = "Naive", y = 0.2, label = "Naive"), parse = TRUE, 
  ...
  geom_text(aes(x = "LL", y = 0.2, label = paste0(expression(w[A]))), parse = TRUE, 
            ...
            size = 2.2,
             position = position_nudge(x = -0.2))+
    geom_text(aes(x = "LC", y = 0.2, label = paste0(expression(w[B](beta == 3)))), parse = TRUE, 
            hjust = +0.05, 
                     vjust = -0.5,
            size = 2.2,
             position = position_nudge(x = -0.2))+
  ...
  geom_text(aes(x = "CC2", y = 0.2, label = paste0(expression(w[D](gamma == 7 ~ "," ~ beta == 1.5)))), parse = TRUE, 
           hjust = +0.05, 
            ...
            size = 2.2,
             position = position_nudge(x = -0.2))+
  geom_text(aes(x = "CC1", y = 0.2, label = paste0(expression(w[D](gamma == 1.5 ~ "," ~ beta == 7)))), parse = TRUE, 
           hjust = +0.05, 
                     vjust = -0.5,
            size = 2.2,
             position = position_nudge(x = -0.2))
...
\begin{figure*}[!hbt]
\centering
\caption{Application of ORB-adjustment sensitivity analysis to example epilepsy data \citep{Copas2019, topiramate}, using selection function \eqref{special1} in (a), selection function \eqref{special3} in (b), and selection function \eqref{special2} in (c).\\[0.5mm]}
<<exampleSENS, echo=FALSE, results='asis', fig.height=7, fig.width=7, message=FALSE, warning=FALSE>>=

#####w=1######

# Plot using ggplot2
g11 <- ggplot(resultsSENS1, aes(x = rho1, y = rho2)) +
  geom_tile(aes(fill = exp(mu))) +
  geom_contour(aes(z = exp(mu)), color = "black", bins = 10) +
  #geom_contour(aes(z = ci_low), color = "blue", linetype = "dashed", bins = 5) +
  #geom_contour(aes(z = ci_up), color = "red", linetype = "dashed", bins = 5) +
  labs(title = "(a) 50% Seizure Frequency Reduction",
       x = "",
       y = expression(beta)) +
  scale_fill_viridis_c(
  name = expression(mu[adj]),
  option = "D",
  n.breaks = 5  # More discrete steps in the legend
)+
  scale_x_continuous(breaks = NULL) +
scale_y_continuous(breaks = seq(min(round(resultsSENS$rho2)), max(round(resultsSENS$rho2)), by = 0.5))+
  theme_minimal()+
  theme(
...
# Plot using ggplot2
g21 <- ggplot(resultsSENS21, aes(x = rho1, y = rho2)) +
  geom_tile(aes(fill = exp(mu))) +
  geom_contour(aes(z = exp(mu)), color = "black", bins = 10) +
  #geom_contour(aes(z = ci_low), color = "blue", linetype = "dashed", bins = 5) +
  #geom_contour(aes(z = ci_up), color = "red", linetype = "dashed", bins = 5) +
  #scale_fill_viridis_c(name = expression(mu[adj])) +
  labs(title = "Seizure Freedom",
       x = "",
       y = expression(beta)) +
  scale_fill_viridis_c(
  name = expression(mu[adj]),
...
#plot_data_sel1 <- lapply(beta_vals, function(beta) {
 # data.frame(
  #  y = y_values,
   # sel = sel1(y_values, beta),
  #  beta = beta
  #)
#}) %>% bind_rows()

# Convert beta to factor for discrete color scale
#plot_data_sel1$beta <- factor(plot_data_sel1$beta, levels = beta_vals)

# Plot
#gsel1 <- ggplot(plot_data_sel1, aes(x = pnorm(-y/sigma), y = sel, color = beta)) +
 # geom_line(size = 1) +
  #labs(
   # title = "",
  #  x = "One-sided p-value",
...
########w=0.5###########
# Plot using ggplot2
g1 <- ggplot(resultsSENS, aes(x = rho1, y = rho2)) +
  geom_tile(aes(fill = exp(mu))) +
  geom_contour(aes(z = exp(mu)), color = "black", bins = 10) +
  #geom_contour(aes(z = ci_low), color = "blue", linetype = "dashed", bins = 5) +
  #geom_contour(aes(z = ci_up), color = "red", linetype = "dashed", bins = 5) +
  ...
  scale_x_continuous(breaks = seq(min(round(resultsSENS$rho1)), max(round(resultsSENS$rho1)), by = 0.5)) +
scale_y_continuous(breaks = seq(min(round(resultsSENS$rho2)), max(round(resultsSENS$rho2)), by = 0.5))+
  theme_minimal()+
  theme(
    legend.title = element_text(size = 8),
...
# Plot using ggplot2
g2 <- ggplot(resultsSENS2, aes(x = rho1, y = rho2)) +
  geom_tile(aes(fill = exp(mu))) +
  geom_contour(aes(z = exp(mu)), color = "black", bins = 10) +
  #geom_contour(aes(z = ci_low), color = "blue", linetype = "dashed", bins = 5) +
  #geom_contour(aes(z = ci_up), color = "red", linetype = "dashed", bins = 5) +
...
#Selection function used
   ...
   # selection05 <- function(y, gamma, beta) {

   #   p <- pnorm(-y/sigma)
   #   ifelse(p <= 0.05, 
   #         1 - (1-0.5)*(p^(gamma))/(0.05^(gamma)),  # Function before p = 0.05
   #         0.5*(p^(-beta))/(0.05^(-beta)))
...
# Add the piece-wise, piecewise2, exponential, and sigmoid function values to the data frame
   # example_combos <- expand.grid(
   #   gamma = c(0.5, 2),
  beta = c(0.5, 2)
   # )

   # plot_data <- purrr::pmap_dfr(example_combos, function(gamma, beta) {
   #   data.frame(
   #     y = y_values,
    #    sel = selection05(y_values, gamma, beta),
    #    gamma = gamma,
    #    beta = beta,
    #    combo = paste0("γ=", gamma, ", β=", beta)
    #  )
   # }) %>%
   #   mutate(
    #    label = paste0("γ=", gamma, ", β=", beta),
    #    style = case_when(
     #     gamma == 0.5 & beta == 0.5 ~ "red_solid",
     #     gamma == 0.5 & beta == 2   ~ "black_dotted",
     #     gamma == 2   & beta == 0.5 ~ "blue_dashdot",
     #     gamma == 2   & beta == 2   ~ "green_solid"
     #   )
    #  )

   # plot_data$style <- factor(plot_data$style,
    #  levels = c("red_solid", "green_solid", "black_dotted", "blue_dashdot")
   # )

# Create the plot


# Create plot with manual style mapping
   # gsel05 <- ggplot(plot_data, aes(x = pnorm(-y/sigma), y = sel, group = style)) +
   #   geom_line(
   #     aes(color = style, linetype = style),
   #     linewidth = 1
   #   ) +
    #  scale_color_manual(
    #    name = expression(paste(gamma, ", ", beta)),
    #    values = c(
    #      red_solid = "yellow",
    #      black_dotted = "black",
    #      blue_dashdot = "blue",
    ...
    #    ),
     #   labels = c(
     #     red_solid = "γ=0.5, β=0.5",
     #     black_dotted = "γ=0.5, β=2",
     #     blue_dashdot = "γ=2, β=0.5",
     #     green_solid = "γ=2, β=2"
    #    )
   #   ) +
...
# Plot using ggplot2
g10 <- ggplot(resultsSENS0, aes(x = rho1, y = rho2)) +
  geom_tile(aes(fill = exp(mu))) +
  geom_contour(aes(z = exp(mu)), color = "black", bins = 10) +
  #geom_contour(aes(z = ci_low), color = "blue", linetype = "dashed", bins = 5) +
  #geom_contour(aes(z = ci_up), color = "red", linetype = "dashed", bins = 5) +
...
# Plot using ggplot2
g20 <- ggplot(resultsSENS20, aes(x = rho1, y = rho2)) +
  geom_tile(aes(fill = exp(mu))) +
  geom_contour(aes(z = exp(mu)), color = "black", bins = 10) +
  #geom_contour(aes(z = ci_low), color = "blue", linetype = "dashed", bins = 5) +
  #geom_contour(aes(z = ci_up), color = "red", linetype = "dashed", bins = 5) +
...
sel0 <- function(y, gamma) {
  
  p <- pnorm(-y/sigma)
  ifelse(p <= 0.05,
         1-(p^gamma)/(0.05^gamma),
         0)
         
}

# Create data for plotting
   # plot_data_sel0 <- lapply(gamma_vals, function(gamma) {
   #   data.frame(
   #     y = y_values,
   #     sel = sel0(y_values, gamma),
   #     gamma = gamma
   #   )
   # }) %>% bind_rows()

# Convert beta to factor for discrete color scale
   # plot_data_sel0$gamma <- factor(plot_data_sel0$gamma, levels = gamma_vals)

# Plot
   # gsel0 <- ggplot(plot_data_sel0, aes(x = pnorm(-y/sigma), y = sel, color = gamma)) +
     # geom_line(size = 1) +
   #   labs(
    #    title = "",
    #    x = "One-sided p-value",
